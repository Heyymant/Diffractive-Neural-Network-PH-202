...
def run(save_loc="mlp/ONN", n_hid=2, loss=Loss.MSE, OD=10, gradient=Gradient.APPROXIMATE):
    ...
    for i in range(n_seeds):
        ...
        classifier = Classifier(network, train_loader, test_loader,
                                n_epochs=20,
                                learning_rate=5e-4,
                                init_weight_mean=0. if OD<=30 else 0.05, init_weight_std=0.1 if OD<=30 else 0.15,
                                loss=loss_str,
                                weight_range=None,
